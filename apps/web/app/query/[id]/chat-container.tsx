...
export const ChatContainer = ({
  ...
  const { isActive, step, stepInProgress } = useTutorial();

  useEffect(() => {
    console.log("isActive", isActive, step, stepInProgress);
    if (isActive && !stepInProgress && TutorialSteps[Number(step)]) {
      const delay = (TutorialSteps[Number(step)]?.duration || []).reduce(
        (acc, d) => acc + d,
        0,
      );
      setTimeout(() => {
        setFollowUps(TutorialSteps[Number(step)]?.choices || []);
      }, delay + 500);
    } else if (isActive && stepInProgress) {
      setFollowUps([]);
    }
  }, [isActive, step, stepInProgress]);
    ...
    >
      <Box ref={scrollRef} sx={{ overflowY: "auto" }}>
        <Box sx={{ flex: 1, pr: 1, pb: 20 }}>
          ...
          {isActive &&
            !stepInProgress &&
            TutorialSteps[Number(step)]?.text.map(
                    ...
                    <TypeAnimation
                      ...
                      sequence={
                        idx === 0
                          ? [text]
                          : [
                              TutorialSteps[Number(step)]?.duration?.[
                                idx - 1
                              ] || 0,
                              text,
                            ]
                  ...
                  followUps.map((f: string) => (
                    <Box
                      ...
                      sx={{
                        ...
                        border: `1px solid ${alpha(mode === "dark" ? "#fff" : "#000", val === f ? 0.8 : 0.5)}`,
                        "&:hover": {
